...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin clear room messages endpoint
  app.delete('/api/admin/rooms/:roomId/messages', async (req, res) => {
    try {
      const { adminUserId } = req.body;
      const { roomId } = req.params;
      ...
      const success = await storage.clearRoomMessages(roomId);
      
      if (success) {
        res.json({ success: true, message: `Messages cleared for room: ${room.name}` });
      } else {
