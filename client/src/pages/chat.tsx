...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Initial room selection effect:`, {
    ...
    if (roomsData?.rooms && roomsData.rooms.length > 0 && !activeRoom && currentUser) {
      ...
      if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Saved room from localStorage:`, savedRoom);
      ...
      if (savedRoom) {
        try {
          ...
          if (foundRoom) {
            if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Restored room from localStorage: ${foundRoom.name}`);
            roomToSet = foundRoom;
          } else {
            if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Saved room not found, using default: ${roomsData.rooms[0].name}`);
          }
        } catch (e) {
          if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Error parsing saved room, using default:`, e);
        }
      } else {
        if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] No saved room, using default: ${roomsData.rooms[0].name}`);
      }

      if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Setting initial active room: ${roomToSet.name} (${roomToSet.id})`);
      ...
      if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Active room saved to localStorage`);
    }
  ...
  useEffect(() => {
    const handlePrivateChatClosed = (event: CustomEvent) => {
      ...
      if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Received private chat closure event for room ${roomId} by ${closedBy?.username}`);
  ...
  const handleRoomSelect = useCallback((room: Room) => {
    ...
    if (previousRoom && previousRoom.id === room.id) {
      if (import.meta.env.MODE === 'development' && import.meta.env.VITE_DEBUG_CHAT) console.log(`[CHAT_PAGE] Same room selected, skipping re-render: ${room.name}`);
      return;
    }
